...
{
  nodes_ = (Node *)rt.arenaAlloc(persistent_arena, sizeof(Node) *
        numInternalNodes(max_leaves));
  num_nodes_ = 0;
  num_allocated_nodes_ = numInternalNodes(max_leaves);

  leaf_entities_ = (BodyID *)rt.arenaAlloc(persistent_arena, 
        sizeof(BodyID) * max_leaves);

  obj_mgr_ = obj_mgr; // FIXME, get rid of this

  leaf_obj_ids_ =(int32_t *)rt.arenaAlloc(
      persistent_arena, sizeof(int32_t) * max_leaves);
  leaf_aabbs_ = (AABB *)rt.arenaAlloc(persistent_arena, 
        sizeof(AABB) * max_leaves);
  leaf_transforms_ = (LeafTransform *)rt.arenaAlloc(
        persistent_arena, sizeof(LeafTransform) * max_leaves);
  leaf_parents_ = (uint32_t *)rt.arenaAlloc(
        persistent_arena, sizeof(uint32_t) * max_leaves);
  sorted_leaves_ = (int32_t *)rt.arenaAlloc(
        persistent_arena, sizeof(int32_t) * max_leaves);
...
int64_t BVH::numInternalNodes(int64_t num_leaves) const
{
  return std::max(divideRoundUp(num_leaves - 1, int64_t(3)), int64_t(1)) +
...
BOT_ALWAYS_INLINE static inline float atomicMinF(float *addr, float value)
{
#ifdef BOT_GPU
  float old;
  if (!signbit(value)) {
...
BOT_ALWAYS_INLINE static inline float atomicMaxF(float *addr, float value)
{
#ifdef BOT_GPU
  float old;
    ...
    float t_max)
{
  ...
  BodyID closest_hit_entity = BodyID::none();
  ...
  if (closest_hit_entity == BodyID::none()) {
    return BodyID::none();
  }
    ...
    Vector3 *hit_normal)
{
  ...
  const int64_t num_faces = convex_mesh.numFaces;
  ...
  for (int64_t face_idx = 0; face_idx < num_faces; face_idx++) {
...
bool BVH::traceRayIntoLeaf(int32_t leaf_idx,
    ...
    Vector3 *hit_normal)
{
  int32_t obj_id = leaf_obj_ids_[leaf_idx];
  ...
  int64_t prim_offset = obj_mgr_->rigidBodyPrimitiveOffsets[obj_id];
  int64_t num_prims = obj_mgr_->rigidBodyPrimitiveCounts[obj_id];

  bool hit_leaf = false;
  for (int64_t i = 0; i < (int64_t)num_prims; i++) {
    int64_t prim_idx = prim_offset + i;
