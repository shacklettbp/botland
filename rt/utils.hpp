...
#define BOT_DEFER(fn) \
  auto BOT_MACRO_CONCAT(_bot_defer_, __COUNTER__) = ::bot::defer([&]() { fn; })
...
BOT_ALWAYS_INLINE constexpr inline int __builtin_clz(int v)
{
    using U = std::make_unsigned_t<int>;
    return sizeof(U) * 8 - std::bit_width(U(v));
}

BOT_ALWAYS_INLINE constexpr inline int __builtin_clzl(long int v)
{
    using U = std::make_unsigned_t<long int>;
    return sizeof(U) * 8 - std::bit_width(U(v));
}

BOT_ALWAYS_INLINE constexpr inline int __builtin_clzll(long long int v)
{
