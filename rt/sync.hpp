...
#define BOT_TSAN_ENABLED (1)
#endif
#endif

#ifdef BOT_TSAN_ENABLED
extern "C" {
extern void __tsan_acquire(void *addr);
extern void __tsan_release(void *addr);
}

#define BOT_TSAN_ACQUIRE(addr) __tsan_acquire(addr)
#define BOT_TSAN_RELEASE(addr) __tsan_release(addr)
#define BOT_TSAN_DISABLE __attribute__((no_sanitize_thread))

#else
#define BOT_TSAN_ACQUIRE(addr)
#define BOT_TSAN_RELEASE(addr)
#define BOT_TSAN_DISABLE
#endif
...
#if defined(__cpp_lib_atomic_ref) or defined(BOT_GPU)
#define BOT_STD_ATOMIC_REF
#endif
template <typename T>
class AtomicRef {
public:
  AtomicRef(T &ref)
#ifndef BOT_STD_ATOMIC_REF
    : addr_(&ref)
#else
    : ref_(ref)
#endif
  ...
  inline bool compare_exchange_strong(T &expected, T desired)
  {
    ...
    return __atomic_compare_exchange_n(
      (ValueT *)addr_, (ValueT *)&expected,
      __builtin_bit_cast(ValueT, desired), false,
      OrderMap<success_order>::builtin,
      OrderMap<failure_order>::builtin);
#else
    return ref_.compare_exchange_strong(expected, desired,
                                      success_order, failure_order);
#endif
