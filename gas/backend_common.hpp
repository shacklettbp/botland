...
struct ResourceTable {
  ...
  static constexpr inline i32 computeChunkSize()
  {
    ...
    constexpr i32 desired_chunk_size = roundToAlignment(
      std::max(BOT_CACHE_LINE, num_hot_bytes), BOT_CACHE_LINE);
  ...
  static constexpr inline i32 NUM_HOT_CHUNKS = 
    roundToAlignment(MAX_NUM_ELEMS, CHUNK_SIZE);

  struct alignas(BOT_CACHE_LINE) HotChunk {
    Hot hot[CHUNK_SIZE];
    u16 gen[CHUNK_SIZE];
  };

  static_assert(sizeof(HotChunk) % BOT_CACHE_LINE == 0);
