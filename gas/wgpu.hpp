...
struct StagingBelt {
  ...
  alignas(BOT_CACHE_LINE) u32 lock;
};
...
struct GPUTmpInputState {
  ...
  alignas(BOT_CACHE_LINE) u64 curTmpStagingRange;
  alignas(BOT_CACHE_LINE) u64 curTmpInputRange;
...
struct TmpParamBlockState {
  alignas(BOT_CACHE_LINE) u32 numLive;
