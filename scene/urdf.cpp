...
static Body::Type urdfToBodyType(URDFJointType type)
{
  switch (type) {
    case URDFJointType::Revolute: {
      return Body::Type::Hinge;
    };

    case URDFJointType::Spherical: {
      return Body::Type::Ball;
    };
    ...
    case URDFJointType::Prismatic: {
      return Body::Type::Slider;
    };

    case URDFJointType::Floating: {
      return Body::Type::FreeBody;
    };
...
struct URDFLoader::Impl {
  ...
  std::vector<BodyNameHash> nameHashes;
...
static URDFModel parseURDF(const std::string &xml_string)
{
  ...
  model.world = URDFLink {
    ...
    .childJointNames = {},
    .childLinkNames = {},
    .parentCom = Vector3::zero(),
    .parentToJoint = Vector3::zero(),
    .parentToChildRot = Quat::id(),
    .jointToChildCom = Vector3::zero(),
    .idx = 0,
  };
    ...
    bool visualize_colliders)
{
  ...
  { // Push the root body
    ...
    BodyDesc body_desc = {
      .type = make_root_free_body ? Body::Type::FreeBody : Body::Type::FixedBody,
  ...
  for (uint32_t i = 1; i < sorted_links.size(); ++i) {
    ...
    BodyDesc body_desc = {
      .type = urdfToBodyType(joint.type),
      .responseType = ResponseType::Dynamic,
      .numVisualObjs = (uint32_t)link.totalNumVisualGeoms(),
      .mass = link.inertial.mass,
      .inertia = { link.inertial.ixx, link.inertial.iyy, link.inertial.izz },
    ...
    total_num_dofs += Body::getTypeDim(body_desc.type);
  ...
  for (auto &joint : model.joints) {
    ...
    Body::Type dof_type = urdfToBodyType(joint.second.type);
    ...
    bool visualize_colliders)
{
  ...
  if (!stream) {
    printf("File %s does not exist", path.c_str());
    assert(false);
  }
