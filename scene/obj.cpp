...
struct OBJLoader::Impl {
  std::vector<SourceMesh> objMeshes;

  // These buffers are iteratively filled while parsing the OBJ
  std::vector<Vector3> curPositions;
  std::vector<Vector3> curNormals;
  std::vector<Vector2> curUVs;
  std::vector<ObjIDX> curIndices;
  std::vector<uint32_t> curFaceCounts;

  // Temporary buffers kept around to avoid reallocations
  std::vector<Vector3> unindexedPositions;
  std::vector<Vector3> unindexedNormals;
  std::vector<Vector2> unindexedUVs;
  std::vector<uint32_t> fakeIndices;
...
bool OBJLoader::Impl::commitMesh(ImportedGeometryAssets &out_assets, const char *name)
{
  ...
  for (const ObjIDX &obj_idx : curIndices) {
    ...
    if (pos_idx >= (int64_t)curPositions.size()) {
    ...
    if (obj_idx.normalIdx > 0) {
      ...
      if (normal_idx >= (int64_t)curNormals.size() ) {
  ...
  vertexRemap.resize(unindexedPositions.size());
  ...
  std::vector<Vector3> new_positions(0);
  new_positions.resize(num_new_verts);
  std::vector<uint32_t> new_indices(0);
  new_indices.resize(fakeIndices.size());
  std::vector<Vector3> new_normals(0);
  std::vector<Vector2> new_uvs(0);
  ...
  if (unindexedNormals.size() > 0) {
    new_normals.resize(num_new_verts);
  ...
  std::vector<uint32_t> face_counts_copy(0);
  face_counts_copy.reserve(curFaceCounts.size());
  ...
  objMeshes.push_back(SourceMesh {
...
bool OBJLoader::Impl::load(const char *path, ImportedGeometryAssets &imported_assets)
{
  ...
  imported_assets.objects.push_back({
      .meshes = { objMeshes.data(), (int64_t)objMeshes.size() },
      });
