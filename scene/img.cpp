...
static std::optional<SourceTexture> stbiImportR8G8B8A8(void *data, size_t num_bytes)
{
    ...
    if (!pixel_data) {
        return std::optional<SourceTexture>{};
    }
...
std::optional<SourceTexture> ImageImporter::importImage(
...
std::optional<SourceTexture> ImageImporter::importImage(const char *path)
{
    ...
    std::optional<SourceTexture> img = importImage(file_data, num_bytes, type_code);
    ...
    Span<const char * const> paths)
{
    ...
    for (CountT i = 0; i < paths.size(); i++) {
        ...
        std::optional<SourceTexture> tex = importImage(path);
        if (!tex.has_value()) {
